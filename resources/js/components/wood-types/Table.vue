<template>
  ...
  <div class="flex-1 container mx-auto p-6">
    ...
    <div class="mb-8 flex justify-between items-start">
      <div>
        <h1 class="text-2xl font-semibold text-black mb-1">Holzarten</h1>
        <p class="text-sm text-gray-500">{{ woodTypes.length }} Artikel</p>
      </div>
  ...
  >
    <div class="bg-white p-6 rounded-sm shadow-lg max-w-md w-full mx-4">
      <h3 class="text-lg font-semibold text-black mb-4">Holzart löschen</h3>
...
const deleteWoodType = async () => {
  ...
  try {
    ...
    showToast('Holzart erfolgreich gelöscht!', 'success');
  } catch (error) {
    console.error('Error deleting wood type:', error);
    showToast('Fehler beim Löschen der Holzart. Bitte versuchen Sie es erneut.', 'error');
  } finally {
