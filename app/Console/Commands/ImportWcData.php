...
use App\Models\WcProductImage;
use App\Models\WcProductVariation;
use Illuminate\Console\Command;

class ImportWcData extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:import-wc-data';
    ...
    {
        $csvPath = storage_path('app/product-export-2.csv');
    ...
    private function createSimpleProduct(array $data): WcProduct
    {
        $attributes = [];
        for ($i = 1; $i <= 5; $i++) {
    ...
    private function createVariableProduct(array $data): WcProduct
    {
        $attributes = [];
        for ($i = 1; $i <= 5; $i++) {
            $attrName = $data["Attribut {$i} Name"] ?? null;
    ...
    private function createVariation(array $data, array $parentProducts): ?WcProductVariation
    {
        ...
        return WcProductVariation::create([
            'wc_product_id' => $parentProduct->id,
            'wc_id' => $data['ID'],
